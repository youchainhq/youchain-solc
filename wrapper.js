...
function setupMethods(soljson) {
  ...
  var compileStandardWrapper = function(input, readCallback) {
    ...
    if (compileJSONCallback !== null) {
      return translateOutput(
        compileJSONCallback(JSON.stringify({sources: sources}), isOptimizerEnabled(input), readCallback),
        libraries
      );
    }
    ...
    if (compileJSONMulti !== null) {
      return translateOutput(
        compileJSONMulti(JSON.stringify({sources: sources}), isOptimizerEnabled(input)),
        libraries
      );
    }
  ...
  return {
    ...
    loadRemoteVersion: function(versionString, cb) {
      ...
      var url = 'https://youchainhq.github.io/solc-bin/bin/soljson-' + versionString + '.js';
      https
