...
tape('CLI', function (t) {
  t.test('--version', function (st) {
    var spt = spawn(st, './youchain-solcjs --version');
    spt.stdout.match(
      RegExp(pkg.version + '(-[^a-zA-A0-9.+]+)?(\\+[^a-zA-Z0-9.-]+)?')
    );
    spt.stderr.empty();
    spt.end();
  });
  ...
  t.test('no parameters', function (st) {
    var spt = spawn(st, './youchain-solcjs');
    spt.stderr.match(/^Must provide a file/);
    spt.end();
  });

  t.test('no mode specified', function (st) {
    var spt = spawn(st, './youchain-solcjs test/' + daodir + '/Token.sol');
  ...
  t.test('--bin', function (st) {
    var spt = spawn(
      st,
      './youchain-solcjs --bin test/' + daodir + '/Token.sol'
    );
    spt.stderr.empty();
    spt.succeeds();
    spt.end();
  ...
  t.test('invalid file specified', function (st) {
    var spt = spawn(st, './youchain-solcjs --bin test/fileNotFound.sol');
  ...
  t.test('incorrect source source', function (st) {
    var spt = spawn(
      st,
      './youchain-solcjs --bin test/fixtureIncorrectSource.sol'
    );
    spt.stderr.match(
  ...
  t.test('--abi', function (st) {
    var spt = spawn(
      st,
      './youchain-solcjs --abi test/' + daodir + '/Token.sol'
    );
    spt.stderr.empty();
    spt.succeeds();
    spt.end();
  ...
  t.test('--bin --abi', function (st) {
    var spt = spawn(
      st,
      './youchain-solcjs --bin --abi test/' + daodir + '/Token.sol'
    );
    spt.stderr.empty();
    spt.succeeds();
    spt.end();
  ...
  t.test('standard json', function (st) {
    var input = {
      ...
      sources: {
        'Contract.sol': {
          content:
            'pragma solidity >=0.4.25; contract Contract { function f() pure public {} }'
        }
      }
    };
    var spt = spawn(st, './youchain-solcjs --standard-json');
